...
/*
    ...
    I had to edit getword for the special case of backslash pipeline.
    When there is a backslash pipeline, getword returns a -50 to differentiate them between a normal pipeline
    Also, the metacharacter # had to be changed to have comment detection working properly
    I had to ungetc once more to deal with no spacing.
*/
int getword(char *w)
{
    int iochar;
        ...
        */
        if(iochar == '\\')
        {
            ...
            if(iochar == ' ' || iochar == '<' || iochar == '>' || iochar == '|' || iochar == '&')
            {
                ...
                if((iochar = getchar()) == '<' || iochar == '>' || iochar == '|' || iochar == '&')
                {
                    ungetc(iochar, stdin);
                    break;
                }
    ...
    */
    ...
    // If the input is a backslash pipeline then it returns -50 to differentiate a backslash pipeline to a normal pipeline
    if(pipeflag > 0 && n == 1)
    {
        c[n] = '\0';
        return -50;
