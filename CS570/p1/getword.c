...
int getword(char *w)
{
    ...
    while((iochar = getchar()) != EOF)
    {
        ...
        */
        ...
        //printf("GREATER_VALUE: %d\n", greater);
        /*
        if(greater == 1)
        {
            printf("in greater if statement\n");
            greater = 0;
            if (c[1] == '>')
            {
                n++;
        ...
        */
        if(iochar == '\\')
        {
            //printf("BACKSLASH: SHOULDNT BE HERE\n");
            backslash++;
            c[n] = iochar;
            iochar = getchar();
            if(iochar == '\n')
